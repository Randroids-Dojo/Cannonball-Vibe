...
| Q-016 | P1 | M4 | Infrastructure | What independent recovery replica protects retained source artifacts from deletion of an entire immutable GitHub Release or repository? | Treat GitHub Releases as the primary store, preserve upstream URLs and hashes, and do not claim disaster recovery until a second durable copy passes a restore drill. | Provider, cost, retention, access control, replication, inventory, and destructive restore-drill evidence. |
