...
| Service brake | S | Left trigger (LT) |
...
deadzone. Left-stick Y is intentionally unused while driving, so pushing the
stick forward cannot accelerate. Rear view is a player-controlled hold action;
reverse does not force a camera change. Releasing rear view returns promptly to
the prior camera orientation without changing camera mode or stored free look.
