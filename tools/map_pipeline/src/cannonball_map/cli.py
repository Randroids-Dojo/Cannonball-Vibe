...
from cannonball_map.continental import (
    ...
    author_continental_endpoint_connectors,
    ...
    derive_continental_junction_geometry,
    derive_continental_lane_topology,
    ...
    validate_continental_endpoint_connectors,
    validate_continental_junction_geometry,
    validate_continental_lane_topology,
...
@app.command("derive-continental-directed-route")
def derive_continental_directed_route_command(
    selection: Path = typer.Option(
        Path("data/routes/continental/route-selection.v1.json"),
        exists=True, file_okay=True, dir_okay=False,
    ),
    ...
    output: Path = typer.Option(
        ...
        help="Directed route lock to write.",
        file_okay=True, dir_okay=False,
    ),
) -> None:
    """Derive the westbound directed edge sequence over the closed corridor.

    Traverses every locked segment from its ADR-0024 from-anchor to its
    to-anchor on exactly the solved graph, records per-element orientation,
    measures, and geodesic stationing, and refuses any traversal that does not
    reproduce the edge-path and chain-connectivity locks to the millimetre. No
    network access; the reciprocal directed carriageway stays a reconstruction
    responsibility under ADR-0014.
    """
    try:
        payload = derive_continental_directed_route_lock(
            selection,
            route_lock,
            transfer_lock,
            policy,
        ...
        typer.echo(f"continental-directed-route-failed: {error}", err=True)
        raise typer.Exit(code=1) from error
    corridor = payload["corridor"]
    authoritative = corridor["authoritative_distance"]
    typer.echo(
        f"continental-directed-route: {output} ({payload['segment_count']} segments, "
        f"{payload['summary']['element_count']} elements, corridor "
        f"{corridor['geodesic_length_miles']} mi geodesic, canonical "
        f"{authoritative['geodesic_length_miles']} mi)"
    )


@app.command("validate-continental-directed-route")
def validate_continental_directed_route_command(
    directed_lock: Path = typer.Argument(
        Path("data/routes/continental/directed-route-lock.v1.json"),
        help="Directed route lock to validate.",
        exists=True, file_okay=True, dir_okay=False,
    ),
    selection: Path = typer.Option(
        Path("data/routes/continental/route-selection.v1.json"),
    ...
    try:
        payload = validate_continental_directed_route_lock(
            directed_lock,
            selection,
            route_lock,
            transfer_lock,
        ...
        typer.echo(f"continental-directed-route-invalid: {error}", err=True)
        raise typer.Exit(code=1) from error
    corridor = payload["corridor"]
    typer.echo(
        f"continental-directed-route-ok: {directed_lock} "
...
@app.command("author-continental-endpoint-connectors")
def author_continental_endpoint_connectors_command(
    directed_lock: Path = typer.Option(
        Path("data/routes/continental/directed-route-lock.v1.json"),
        exists=True, file_okay=True, dir_okay=False,
    ),
    ...
    catalog: Path = typer.Option(DEFAULT_CATALOG, exists=True, file_okay=True, dir_okay=False),
    output: Path = typer.Option(
        Path("data/routes/continental/endpoint-connector-lock.v1.json"),
        help="Endpoint connector lock to write.",
        file_okay=True, dir_okay=False,
    ),
) -> None:
    ...
    """
    try:
        payload = author_continental_endpoint_connectors(
            directed_lock,
            selection,
            route_lock,
            transfer_lock,
        ...
        typer.echo(f"continental-endpoint-connectors-failed: {error}", err=True)
        raise typer.Exit(code=1) from error
    summary = payload["summary"]
    typer.echo(
        f"continental-endpoint-connectors: {output} "
        f"({summary['connector_count']} connectors, "
        f"{summary['authored_geodesic_m']} m authored, "
        f"{summary['gates_passed']} gates passed, "
        f"{summary['corner_site_count']} corner sites)"
    )


@app.command("validate-continental-endpoint-connectors")
def validate_continental_endpoint_connectors_command(
    connector_lock: Path = typer.Argument(
        Path("data/routes/continental/endpoint-connector-lock.v1.json"),
        help="Endpoint connector lock to validate.",
        exists=True, file_okay=True, dir_okay=False,
    ),
    directed_lock: Path = typer.Option(
        Path("data/routes/continental/directed-route-lock.v1.json"),
    ...
    try:
        payload = validate_continental_endpoint_connectors(
            connector_lock,
            directed_lock,
            selection,
            route_lock,
        ...
        typer.echo(f"continental-endpoint-connectors-invalid: {error}", err=True)
        raise typer.Exit(code=1) from error
    summary = payload["summary"]
    typer.echo(
        f"continental-endpoint-connectors-ok: {connector_lock} "
        f"({summary['connector_count']} connectors, "
        f"{summary['authored_geodesic_m']} m authored, "
        f"{summary['gates_passed']} gates passed)"
    )
...
def derive_continental_westbound_carriageway_command(
    ...
    the ADR-0018 gates. The schema-2 revision replaces the traversed fill
    chords with their seam-registered conflated spans, integrates the
    authored endpoint connector lock, and publishes the ADR-0024
    portal-to-portal run length. Bulk geometry stays in the ignored cache.
    """
    try:
    ...
    typer.echo(
        ...
        f"{summary['span_replacement_count']} span replacements, "
        f"{summary['corner_site_count']} corner sites, "
        f"run length {run_length['canonical_portal_to_portal_miles']} mi)"
    )
...
@app.command("derive-continental-junction-geometry")
def derive_continental_junction_geometry_command(
    ...
    carriageway_cache: Path = typer.Option(
        Path(".tools/continental/carriageway"),
        help="Carriageway geometry cache written by "
        "derive-continental-westbound-carriageway.",
        exists=True, file_okay=False, dir_okay=True,
    ),
    output: Path = typer.Option(
        Path("data/routes/continental/junction-geometry-lock.v1.json"),
        help="Junction geometry lock to write.",
        file_okay=True, dir_okay=False,
    ),
) -> None:
    """Derive the junction transfer geometry lock (ADR-0013/ADR-0018).

    Refuses unless the carriageway cache reproduces the committed westbound
    carriageway lock's digests, then derives the directed westbound movement
    through every cross-segment junction and authors the two backtrack
    turn-around loops from the proven reciprocal-pair seam poses, through
    the full gate battery. The lock carries the complete movement geometry,
    so validation needs no cache.
    """
    try:
        payload = derive_continental_junction_geometry(
            carriageway_lock,
            conditioned_lock,
            elevation_lock,
            dem_lock,
        ...
        typer.echo(f"continental-junction-geometry-failed: {error}", err=True)
        raise typer.Exit(code=1) from error
    summary = payload["summary"]
    typer.echo(
        f"continental-junction-geometry: {output} "
        f"({summary['movement_count']} movements, "
        f"{summary['through_transfer_count']} through transfers, "
        f"{summary['turnaround_transfer_count']} turn-arounds, "
        f"{summary['corner_site_count']} corner sites, "
        f"{summary['gates_passed']} gates passed)"
    )


@app.command("validate-continental-junction-geometry")
def validate_continental_junction_geometry_command(
    junction_lock: Path = typer.Argument(
        Path("data/routes/continental/junction-geometry-lock.v1.json"),
        help="Junction geometry lock to validate.",
        exists=True, file_okay=True, dir_okay=False,
    ),
    carriageway_lock: Path = typer.Option(
        Path("data/routes/continental/westbound-carriageway-lock.v1.json"),
    ...
    try:
        payload = validate_continental_junction_geometry(
            junction_lock,
            carriageway_lock,
            conditioned_lock,
            elevation_lock,
        ...
        typer.echo(f"continental-junction-geometry-invalid: {error}", err=True)
        raise typer.Exit(code=1) from error
    summary = payload["summary"]
    typer.echo(
        f"continental-junction-geometry-ok: {junction_lock} "
        f"({summary['movement_count']} movements, "
        f"{summary['cross_segment_junction_anchor_count']} junction anchors, "
        f"{summary['turnaround_anchor_count']} turn-around anchors, "
        f"{summary['gates_passed']} gates passed)"
    )


@app.command("derive-continental-lane-topology")
def derive_continental_lane_topology_command(
    ...
    carriageway_cache: Path = typer.Option(
        Path(".tools/continental/carriageway"),
        help="Ignored cache holding the bulk carriageway geometry.",
        exists=True, file_okay=False, dir_okay=True,
    ),
    output: Path = typer.Option(
        Path("data/routes/continental/lane-topology-lock.v1.json"),
        help="Lane topology lock to write.",
        file_okay=True, dir_okay=False,
    ),
) -> None:
    """Derive the continental lane topology lock (ADR-0011/ADR-0013).

    Refuses unless the carriageway cache reproduces the committed lock,
    censuses the locked NHPN schema for lane attribution, authors the
    speed-designed corner refinements over every recorded exception site,
    eases and censuses the vertical profile, and builds the complete lane
    plan (sections, ADR-0013 transitions, gores, connectors, authored
    grade separations) through the deferred ADR-0018 design gates.
    """
    try:
        payload = derive_continental_lane_topology(
            junction_lock,
            carriageway_lock,
            conditioned_lock,
            elevation_lock,
        ...
        typer.echo(f"continental-lane-topology-failed: {error}", err=True)
        raise typer.Exit(code=1) from error
    summary = payload["summary"]
    typer.echo(
        f"continental-lane-topology: {output} "
        ...
        f"{summary['vertical_class_80_site_count']} vertical sites, "
        f"{summary['gates_passed']} gates passed)"
    )


@app.command("validate-continental-lane-topology")
def validate_continental_lane_topology_command(
    lane_lock: Path = typer.Argument(
        Path("data/routes/continental/lane-topology-lock.v1.json"),
        help="Lane topology lock to validate.",
        exists=True, file_okay=True, dir_okay=False,
    ),
    junction_lock: Path = typer.Option(
        Path("data/routes/continental/junction-geometry-lock.v1.json"),
    ...
    try:
        payload = validate_continental_lane_topology(
            lane_lock,
            junction_lock,
            carriageway_lock,
            conditioned_lock,
        ...
        typer.echo(f"continental-lane-topology-invalid: {error}", err=True)
        raise typer.Exit(code=1) from error
    summary = payload["summary"]
    typer.echo(
        f"continental-lane-topology-ok: {lane_lock} "
        f"({summary['section_count']} sections, "
        f"{summary['refinement_count']} corner refinements, "
        f"{summary['lane_connector_count']} lane connectors, "
        f"{summary['gates_passed']} gates passed)"
