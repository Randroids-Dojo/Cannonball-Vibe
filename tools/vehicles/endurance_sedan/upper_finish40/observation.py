...
from ..qa.upper_finish_report import COMPANION, INPUTS, NAMES, POLICY, revision_requested, require
...
def observe(root, construction):
    ...
    files = [proof[key] for key in ('checkpoint', 'requested', 'constructor', 'generator', 'encoder', 'design')]
    ...
    records.verify_rows(files, root)
    return {'schema': 'source-upper-observation40.v1', 'checkpoint': records.file_row(source),
