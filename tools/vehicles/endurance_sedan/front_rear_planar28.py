"""Exact boundary triangulation of three declared planar fascia wall domains."""
from collections import defaultdict
from fractions import Fraction
import bpy
from . import hole_triangulation48

PLANES={2.2300000190734863}
...
def apply(obj,encode,*,ideal_stream=None):
    ...
    planar_points=[(p[0],p[2],p[1]) for p in points]
    selected={p.index for p in old.polygons if len({points[i][1] for i in p.vertices})==1 and points[p.vertices[0]][1] in PLANES}
        ...
        while stack:
            ...
            for a,b in zip(ids,ids[1:]+ids[:1]):
                for j in edge_faces[tuple(sorted((a,b)))]:
                    if j in pending and points[old.polygons[j].vertices[0]][1]==points[ids[0]][1]:pending.remove(j);stack.append(j)
        components.append(sorted(part))
    faces=[];materials=[];smooth=[];normal_targets=[];uvs={layer.name:[] for layer in old.uv_layers}
    if set(uvs)!={'SurfaceMeters'}:raise ValueError('Unknown rear closure chart')
    ...
    for component in components:
        ...
        for i in component:
            ...
            for index,vertex in zip(p.loop_indices,p.vertices):
                chart={layer.name:(points[vertex][0]/.25,points[vertex][2]/.25) for layer in old.uv_layers}
        ...
        pending=set(successor);loops=[]
        while pending:
            first=min(pending);boundary=[first];pending.remove(first);following=successor[first]
            while following!=first:
                if following not in pending:raise ValueError('Open, shared or branched planar loops')
                pending.remove(following);boundary.append(following);following=successor[following]
            loops.append(boundary)
        boundary=loops[0];first=boundary[0]
        interior=set(uv_at)-{i for loop in loops for i in loop}
        new,sign,cdt=hole_triangulation48.triangulate(loops,interior,planar_points)
        if len(new)!=len(component):raise ValueError('Planar triangulation changed triangle count')
        for ids in new:
            append(ids,next(iter(mats)),False,[(0.,-float(sign),0.)]*3,
                   {name:[uv_at[i][name] for i in ids] for name in uvs})
        proofs.append({'old_faces':component,'boundary':boundary,'boundary_loops':loops,'native_cdt':cdt,'original_interior_vertices':sorted(interior),'new_triangles':new,'plane_y_m':points[first][1],'normal_y':-sign})
    mesh=bpy.data.meshes.new(obj.name+'CompleteRearClosure')
    ...
    return {'selected_planar_faces':len(selected),'components':proofs,'positions_and_boundary_indices_exact':True,
            ...
            'authored_uv_field':{'channel':'SurfaceMeters','formula':'(X/0.25m,Z/0.25m)','old_interior_field_preserved':False}}
