...
def _simplify_group(name, members, parent, collection, material, requested_ratio, *,
                   ...
                   include_rear_door_quarter=False, component_requested_ratios=None):
    ...
    component_requested_ratios = dict(component_requested_ratios or {})
    if set(component_requested_ratios) - {obj.name for obj in members}:
        raise ValueError('Unknown source component ratio override')
    for value in component_requested_ratios.values():
        ratios(value, include_rear_door_quarter)
    built = []
    reports = []
    original_names = {}
    for original in sorted(members, key=lambda obj: obj.name):
        component_ratio = component_requested_ratios.get(original.name, requested_ratio)
        schedule = ratios(component_ratio, include_rear_door_quarter)
        ...
        reports.append({'source_component': original.name, 'requested_ratio': component_ratio,
...
def simplify_group(name, members, parent, collection, material, requested_ratio, *,
    ...
    try:
        result = _simplify_group(name, members, parent, collection, material,
            ...
            component_requested_ratios=component_requested_ratios)
